...
-- In 5.sql, write a SQL query to find cities with 3 or fewer public schools. Your query should return the
--  names of the cities and the number of public schools within them, ordered from greatest number of public 
-- --  schools to least. If two cities have the same number of public schools, order them alphabetically.
...
-- 6.sql
-- DESE wants to assess which schools achieved a 100% graduation rate. In 6.sql, write a SQL query to find 
-- the names of schools (public or charter!) that reported a 100% graduation rate.

-- SELECT name FROM schools WHERE id IN (
    -- SELECT school_id FROM graduation_rates WHERE graduated = 100);

-- 7.sql
-- DESE is preparing a report on schools in the Cambridge school district. In 7.sql, write a SQL query to find
--  the names of schools (public or charter!) in the Cambridge school district. Keep in mind that Cambridge, 
--  the city, contains a few school districts, but DESE is interested in the district whose name is “Cambridge.”

-- SELECT name,district_id FROM schools WHERE district_id = (
--     SELECT id FROM districts WHERE name = 'Cambridge'
-- );


-- 8.sql
-- A parent wants to send their child to a district with many other students. In 8.sql, write a SQL query to 
-- display the names of all school districts and the number of pupils enrolled in each.

-- from expenditures get number of pupiles in each district
...
-- 9.sql
-- Another parent wants to send their child to a district with few other students. In 9.sql, write a SQL 
-- query to find the name (or names) of the school district(s) with the single least number of pupils. Report only the name(s).
...
FROM "districts" AS d
    JOIN "expenditures" AS e ON "districts".id = "expenditures".district_id
...
-- 11.sql
-- Is there a relationship between school expenditures and graduation rates? In 11.sql, write a SQL query to display 
-- the names of schools, their per-pupil expenditure, and their graduation rate. Sort the schools from greatest per-pupil 
-- expenditure to least. If two schools have the same per-pupil expenditure, sort by school name.
