...
-- to_address = 728 Maple Place
-- action pick up timestamp 2 weeks ago from latest date
-- not dropped off
-- con


-- Retrieve package information sent from '109 Tileston Street' to '728 Maple Place'
SELECT 
    p.id AS package_id,
    p.contents,
    s.action,
...
FROM 
    packages p
JOIN 
    addresses a_from ON p.from_address_id = a_from.id
JOIN 
    addresses a_to ON p.to_address_id = a_to.id
LEFT JOIN 
