...
-- The list goes on. So here’s what we track: The name of the airline
...
-- Flights
-- We serve as many as 1,000 flights daily. To ensure that our passengers are 
-- never left wondering, we need to give them all the critical details about 
-- their flight. Here’s what we’d like to store:

-- The flight number. For example, “900”. Just know that we sometimes 
-- re-use flight numbers.

-- The airline operating the flight. You can keep it simple and assume 
-- one flight is operated by one airline.
...
CREATE TABLE IF NOT EXISTS
    "flights" (
        "id"            INTEGER,
        "flight_number" TEXT,-- non-unique
        "airline_id"    INTEGER,
        "departure"     NUMERIC,
        "arrival_date"       NUMERIC, --minutes date and time
        "arrival_time"       NUMERIC, --minutes date and time
